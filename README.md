...
- **`src/components/`**: Компоненти React, такі як `PdfViewer.js`.
- **`src/context/`**: Контекст для управління глобальним станом (наприклад, URL PDF).
- **`src/services/`**: Сервіси для взаємодії з API.
- **`__mocks__/`**: Моки для тестів, зокрема для ігнорування CSS у Jest.
...
- **Функціональність**:
  ...
  - Кнопка для запуску конвертації через `convertTextToPdf`.
  - Відображення історії конвертацій та PDF через `PdfViewer`.
- **Залежності**: `PdfContext`, `PdfViewer`, `convertTextToPdf`.

### 2. **`src/components/PdfViewer.js`**

- **Опис**: Компонент для відображення PDF-файлів у браузері.
- **Функціональність**:
  - Використовує `@react-pdf-viewer/core` для рендерингу PDF.
  - Приймає `pdfUrl` як пропс і відображає відповідний файл.
- **Залежності**: `@react-pdf-viewer/core`, `pdfjs-dist`.

### 3. **`src/context/PdfContext.js`**

- **Опис**: Контекст для управління станом PDF-файлів та історії конвертацій.
- **Функціональність**:
  - Зберігає `pdfUrl` (URL згенерованого PDF) та історію конвертацій.
...
- **`package.json`**: Містить залежності, скрипти (`npm start`, `npm run test`) та конфігурацію Jest.
- **`.babelrc`**:
  ```json
  {
